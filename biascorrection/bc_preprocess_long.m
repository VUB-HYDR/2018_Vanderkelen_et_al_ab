...
time_begin_ev_obs  = [1993, 1, 1, 0,0,0]; %
...
% Define timebounds of intersection obs and historical and rcp data

time_begin_hist_obs  = [1983, 1, 1, 0,0,0]; % date first PERSIANN available
time_end_hist_obs    = [2004,12,31,23,0,0]; % date of last historical run

date_vec_hist_obs = datevec(datenum(time_begin_hist_obs ):1:datenum(time_end_hist_obs ));
date_hist_obs = date_vec_hist_obs(:,1:3); 

% load observed 
load('P_wb_obs_1983-2014.mat')
load('E_wb_obs_1983-2014.mat')
load('Qin_wb_obs_1983-2014.mat')
